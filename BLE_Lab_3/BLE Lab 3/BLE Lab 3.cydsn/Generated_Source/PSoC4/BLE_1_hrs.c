...
void CyBle_HrscWriteResponseEventHandler(const CYBLE_CONN_HANDLE_T *eventParam)
{
    if((NULL != CyBle_HrsApplCallback) && (CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE != cyBle_hrscReqHandle))
    {
        ...
        {
            CYBLE_HRS_DESCR_VALUE_T locDescrIndex;
            
            locDescrIndex.connHandle = *eventParam;
            locDescrIndex.charIndex = CYBLE_HRS_HRM;
            locDescrIndex.descrIndex = CYBLE_HRS_HRM_CCCD;
            locDescrIndex.value = NULL;

            cyBle_hrscReqHandle = CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE;
            cyBle_eventHandlerFlag &= (uint8)~CYBLE_CALLBACK;
            CyBle_HrsApplCallback((uint32)CYBLE_EVT_HRSC_WRITE_DESCR_RESPONSE, &locDescrIndex);
        }
    }
}
